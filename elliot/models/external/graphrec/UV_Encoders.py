...
class UV_Encoder(nn.Module, ABC):
    ...
    def forward(self, nodes):
        ...
        tmp_history_r = []
        for node in nodes:
            if node.item() not in self.history_uv_lists:
                tmp_history_uv.append([])
            else:
                tmp_history_uv.append(self.history_uv_lists[node.item()])
            if node.item() not in self.history_r_lists:
                tmp_history_r.append([])
            else:
                tmp_history_r.append(self.history_r_lists[node.item()])
