...
def main():
    ...
    traindata_to_store = np.vstack((traindata, validdata)).tolist()
    testdata_to_store = testdata.tolist()

    social_connections = []

    for source in social_adj_lists:
        for target in social_adj_lists[source]:
            social_connections.append((source, target))

    social_connections = list(set(social_connections))

    with open('./data/' + args.dataset + '/' + args.dataset + '_ratings_final_train.tsv', 'w') as f:
        for entry in traindata_to_store:
            f.write(str(entry[0]) + '\t' + str(entry[1]) + '\t' + str(entry[2]) + '\n')

    with open('./data/' + args.dataset + '/' + args.dataset + '_ratings_final_test.tsv', 'w') as f:
        for entry in testdata_to_store:
            f.write(str(entry[0]) + '\t' + str(entry[1]) + '\t' + str(entry[2]) + '\n')

    with open('./data/' + args.dataset + '/' + args.dataset + '_social_connections_final.tsv', 'w') as f:
        for entry in social_connections:
